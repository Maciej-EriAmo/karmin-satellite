#!/usr/bin/env python3
"""
Public space weather (NOAA SWPC) — engine.solar.weather.
...
class SpaceWeatherClient:
    ...
    def _snap_from_dict(self, snap_d: dict) -> Optional[SpaceWeatherSnapshot]:
        try:
            return SpaceWeatherSnapshot(
                **{
                    k: snap_d[k]
    ...
    def _cache_series(self, cached: Optional[dict]) -> Dict[str, List[dict]]:
        if not cached:
            return {"xray": [], "kp": [], "f107": []}
        if cached.get("series"):
            return dict(cached["series"])
        return series_from_bundle(cached.get("bundle") or {})

    def fetch(self, *, force: bool = False) -> SpaceWeatherSnapshot:
        """Return snapshot: live → fresh cache → stub."""
        snap, _ = self.fetch_with_series(force=force)
        return snap

    def fetch_with_series(
        self, *, force: bool = False
    ) -> Tuple[SpaceWeatherSnapshot, Dict[str, List[dict]]]:
        """Snapshot plus compact index series for H3 horizon trends."""
        empty_series: Dict[str, List[dict]] = {"xray": [], "kp": [], "f107": []}

        if not force:
            cached = self._read_cache()
            if cached and (time.time() - float(cached.get("saved_at") or 0)) < self.cache_ttl_sec:
                snap = self._snap_from_dict(cached.get("snapshot") or {})
                if snap is not None:
                    return snap, self._cache_series(cached)
        ...
        if not self.allow_network:
            ...
            if cached and cached.get("snapshot"):
                ...
                snap = self._snap_from_dict(snap_d)
                if snap is not None:
                    return snap, self._cache_series(cached)
            return quiet_stub(reason="network disabled, no cache"), empty_series
        ...
        if not f107_rows and not xray_rows and not kp_rows:
            ...
            if cached and cached.get("snapshot"):
                ...
                if snap is not None:
                    return snap, self._cache_series(cached)
            return quiet_stub(reason="; ".join(errors)[:180] or "fetch failed"), empty_series
        ...
        full_bundle = {"f107": f107_rows, "xray": xray_rows, "kp": kp_rows}
        series = series_from_bundle(full_bundle)
        snap.raw_meta = dict(snap.raw_meta or {})
        snap.raw_meta["series_n"] = {k: len(v) for k, v in series.items()}
        # cache: short tails for debug + full compact series for H3
        slim = {
            "f107": (f107_rows[-8:] if isinstance(f107_rows, list) else f107_rows),
            "xray": (xray_rows[-48:] if isinstance(xray_rows, list) else xray_rows),
            "kp": (kp_rows[-16:] if isinstance(kp_rows, list) else kp_rows),
        }
        try:
            self._write_cache(snap, bundle=slim, series=series)
        except Exception:
            pass
        return snap, series


def series_from_bundle(bundle: Optional[dict]) -> Dict[str, List[dict]]:
    """
    Compact time series for H3 predict trends.
    Returns {xray:[{time_tag,flux}], kp:[{time_tag,Kp}], f107:[{time_tag,flux}]}.
    """
    b = bundle or {}
    out: Dict[str, List[dict]] = {"xray": [], "kp": [], "f107": []}

    xrows = list(b.get("xray") or [])
    long = [
        r
        for r in xrows
        if isinstance(r, dict)
        and str(r.get("energy") or "") in ("0.1-0.8nm", "1-8A", "1.0-8.0A", "")
    ]
    pool = long if long else [r for r in xrows if isinstance(r, dict)]
    for r in pool[-48:]:
        try:
            flux = r.get("flux")
            if flux is None:
                flux = r.get("observed_flux")
            if flux is None:
                continue
            out["xray"].append(
                {"time_tag": str(r.get("time_tag") or ""), "flux": float(flux)}
            )
        except (TypeError, ValueError):
            continue

    krows = list(b.get("kp") or [])
    for r in krows:
        if isinstance(r, dict) and "Kp" in r:
            try:
                out["kp"].append(
                    {"time_tag": str(r.get("time_tag") or ""), "Kp": float(r["Kp"])}
                )
            except (TypeError, ValueError):
                continue
        elif isinstance(r, list) and len(r) >= 2:
            try:
                float(r[1])  # skip header
                out["kp"].append({"time_tag": str(r[0]), "Kp": float(r[1])})
            except (TypeError, ValueError):
                continue
    out["kp"] = out["kp"][-16:]

    frows = [r for r in (b.get("f107") or []) if isinstance(r, dict)]
    for r in frows[-8:]:
        try:
            out["f107"].append(
                {
                    "time_tag": str(r.get("time_tag") or ""),
                    "flux": float(r["flux"]),
                }
            )
        except (KeyError, TypeError, ValueError):
...
def get_space_weather_bundle(
    *,
    force: bool = False,
    offline: bool = False,
    cache_path: Optional[Path] = None,
) -> Tuple[SpaceWeatherSnapshot, Dict[str, List[dict]]]:
    """Snapshot + compact series for H3 predict (from live fetch or cache)."""
    client = SpaceWeatherClient(
        cache_path=cache_path,
        allow_network=not offline,
    )
    return client.fetch_with_series(force=force)
